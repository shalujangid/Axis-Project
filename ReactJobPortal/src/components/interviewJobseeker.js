...
class InterviewJobseeker extends Component {
    constructor(props) {
        super(props)
        this.state = {
            interview: []
    ...
    componentDidMount() {
        ...
        UserService.getallListInterviewOfSeeker(seeker_id).then((res) => {
            this.setState({
                interview: res.data
            });
        });
                    ...
                    <table className="myt table table-striped table-bordered shadow-lg p-3 mb-5 bg-white rounded" style={{ margin: "10px", paddingRight: '20px', width: '75%', height: '50%', fontSize: '14px' }} >
                        <thead className="thead-bg" style={{ "background-color": "#0a193d", "color": "white" }}>
                            <tr>
                                ...
                                <th>Company Name</th>
                                <th>Interviewer Contact Number</th>
                                <th>Job Title</th>
                            ...
                            this.state.interview.map(
                                interview =>
                                    <tr key={interview.id} class="mytr">
                                        ...
                                        <td>{interview.job.company.company_name}</td>
                                        <td>{interview.createdBy.contact_number}</td>
...
export default withRouter(InterviewJobseeker);
